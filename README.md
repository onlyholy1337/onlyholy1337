<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=6,12,19,24,30&height=200&section=header&text=%D0%97%D0%94%D0%90%D0%A0%D0%9E%D0%92%D0%90%D0%90%D0%90%2C%20%D1%8F%20onlyholy%20%E2%9D%A4&fontSize=36&fontAlignY=30&desc=%D0%B0%D0%BA%D0%B0%20%D0%B2%D1%81%D0%B5%D0%B2%D1%8B%D1%88%D0%BD%D0%B8%D0%B9%20%D0%B5%D0%B1%D1%8B%D1%80%D1%8C%20%3AD&descSize=20&descAlignY=52&animation=twinkling&fontColor=ffffff" />
</p>

<p align="center">
  <b>Telegram Bot Developer / Автоматизация / Скрипты</b><br>
  <sub>50+ выполненных заказов на FunPay • пишу ботов пока ты спишь</sub>
</p>

<p align="center">
  <a href="https://funpay.com/users/4210009/"><img src="https://img.shields.io/badge/FunPay-Заказать-orange?style=for-the-badge" /></a>
  <a href="https://t.me/hatenightmares"><img src="https://img.shields.io/badge/Telegram-@hatenightmares-blue?style=for-the-badge&logo=telegram&logoColor=white" /></a>
</p>

---

### 🛠 Чем я тут занимаюсь

```python
class OnlyHoly:
    name = "onlyholy aka всевышний ебырь"
    role = "Telegram Bot Developer"
    
    languages = ["Python", "Node.js", "C#"]
    frameworks = ["aiogram 3", "Pyrogram", "Telethon", "FastAPI"]
    databases = ["PostgreSQL", "SQLite", "MongoDB"]
    tools = ["Docker", "Linux", "Git"]
    
    fun_fact = "написал больше ботов чем сообщений в личке"
    
    def get_order(self):
        return "https://funpay.com/users/4210009/"
```

### ⚡ Стек

<p align="center">
  <img src="https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white" />
  <img src="https://img.shields.io/badge/aiogram_3-2CA5E0?style=flat-square&logo=telegram&logoColor=white" />
  <img src="https://img.shields.io/badge/Pyrogram-FF4500?style=flat-square&logo=telegram&logoColor=white" />
  <img src="https://img.shields.io/badge/Telethon-0088CC?style=flat-square&logo=telegram&logoColor=white" />
  <img src="https://img.shields.io/badge/Node.js-339933?style=flat-square&logo=nodedotjs&logoColor=white" />
  <img src="https://img.shields.io/badge/C%23-239120?style=flat-square&logo=csharp&logoColor=white" />
  <img src="https://img.shields.io/badge/FastAPI-009688?style=flat-square&logo=fastapi&logoColor=white" />
  <img src="https://img.shields.io/badge/PostgreSQL-336791?style=flat-square&logo=postgresql&logoColor=white" />
  <img src="https://img.shields.io/badge/SQLite-003B57?style=flat-square&logo=sqlite&logoColor=white" />
  <img src="https://img.shields.io/badge/MongoDB-47A248?style=flat-square&logo=mongodb&logoColor=white" />
  <img src="https://img.shields.io/badge/Docker-2496ED?style=flat-square&logo=docker&logoColor=white" />
  <img src="https://img.shields.io/badge/Linux-FCC624?style=flat-square&logo=linux&logoColor=black" />
</p>

---

### 🤖 Что я делаю (и делаю хорошо)

```
 Telegram-боты      ████████████████████████░░  90%  ← тут я живу
 Скрипты / Софт     ██████████░░░░░░░░░░░░░░░░   8%  ← автоматизирую всё
 Отдых              █░░░░░░░░░░░░░░░░░░░░░░░░░   2%  ← когда-нибудь
```

| Направление | Что именно |
|-------------|-----------|
| **Telegram-боты** | Боты-магазины, автовыдача, оплата Stars, юзерботы, парсеры, бизнес-боты |
| **Mini-Apps** | Мини-игры, веб-приложения внутри Telegram |
| **Юзерботы** | Pyrogram/Telethon — автоматизация аккаунта, рассылки, мониторинг |
| **Скрипты** | Автоматизация FunPay/Playerok, парсеры, чекеры |
| **Десктоп** | WPF/WinForms приложения на C# |

---

### 📂 Проекты

<a href="https://github.com/onlyholy1337/telegram-stars-bot">
  <img align="center" src="https://github-readme-stats.vercel.app/api/pin/?username=onlyholy1337&repo=telegram-stars-bot&theme=tokyonight&hide_border=true" />
</a>
<a href="https://github.com/onlyholy1337/GiftHunter">
  <img align="center" src="https://github-readme-stats.vercel.app/api/pin/?username=onlyholy1337&repo=GiftHunter&theme=tokyonight&hide_border=true" />
</a>

---

### 💬 Цитаты моих заказчиков

> «Сделай бота за 500 рублей, у меня брат программист, он сказал это на 5 минут»
> 
> — каждый второй

> «А можно ещё вот это, вот это и вот это? Ну типа бесплатно, мы ж договорились»
>
> — каждый первый

---

<p align="center">
  <b>нужен бот? скрипт? чудо? пиши</b><br><br>
  <a href="https://funpay.com/users/4210009/">🛒 FunPay</a> •
  <a href="https://t.me/hatenightmares">💬 Telegram</a>
  <br><br>
  <sub>если я не ответил — значит пишу очередного бота. подожди.</sub>
</p>

<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=6,12,19,24,30&height=100&section=footer" />
</p>
